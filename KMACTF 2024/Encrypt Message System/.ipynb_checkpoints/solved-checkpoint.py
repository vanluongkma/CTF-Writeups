...
coefficients = literal_eval(io.recvline()[:-1].decode())
...
io.sendline(msg2)
io.recvuntil(b'x: ')
io.sendline(x_tmp.encode()) 


enc2 = bytes.fromhex(io.recvline()[:-1].decode())
nonce1 , ct1 , tag1 = dec(enc)
nonce2 , ct2, tag2 = dec(enc2)
...
forges = list(chachapoly1305_forgery_attack(b'', ct1, tag1, 
                                            b'', ct2, tag2, 
                                            msg1, 
                                            msg, b''))
