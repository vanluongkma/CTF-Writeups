...
while True:
    try:
        f = remote("157.15.86.73", 1305, level = 'debug')
        f.recvuntil(b'p = ')
        p = int(f.recvline()[:-1].decode())

        f.sendline(b'1')
        f.recvuntil(b'coefficients = ')
        coefficients = eval(f.recvline().strip().decode())  
         
        msg = b'give me the flag'
        msg1 = secrets.token_bytes(32).hex().encode()
        msg2 = secrets.token_bytes(32).hex().encode()

        P = PolynomialRing(GF(p), 'x')
        x = P.gen()
        at_x = sum([coefficients[i]*x**i for i in range(len(coefficients))])
        at_x1= at_x.roots()[0][0]

        f.sendline(msg1)
        f.sendlineafter(b"x: ", str(at_x1).encode())
        enc1 = bytes.fromhex(f.recvline().decode())
        ...
        x = P.gen()
        at_x = sum([coefficients[i]*x**i for i in range(len(coefficients))])
        at_x2 = at_x.roots()[0][0]

        f.sendline(msg2)
        f.sendlineafter(b"x: ", str(at_x2).encode())
        enc2 = bytes.fromhex(f.recvline().decode())

        nonce1 , ct1, tag1 = enc1[:12], enc1[12:-16], enc1[-16:]
        nonce2 , ct2, tag2 = enc2[:12], enc2[12:-16], enc2[-16:]

        forges = list(chachapoly1305_forgery_attack(b'', ct1, tag1, b'', ct2, tag2, msg1, msg, b''))

        ct , tag = forges[0][0] , forges[0][1]
        send_tmp = nonce1 + ct + tag 

        f.sendline(b'2')
        f.sendline(send_tmp.hex())
        if b"KMACTF{" in f.recvline():
            f.close()
            break 
    except: 
        continue